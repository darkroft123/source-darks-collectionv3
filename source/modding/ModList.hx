...
class ModList {
	...
	public static function getActiveMods(modsToCheck:Array<String>):Array<String> {
		...
		for (modName in modsToCheck) {
			if (getModEnabled(modName) && modName != Options.getData("curMod") && modName != "Dark's Collection")
				activeMods.push(modName);
		}
		activeMods.push("Dark's Collection");
		if (Options.getData("curMod") != "Dark's Collection")
